...
/// The AttestationDataService is responsible for downloading attestation data at a given slot.
...
impl<T: SlotClock> AttestationDataService<T> {
    ...
    /// Download attestation data for `request_slot`, trying `candidate_beacon_node` first
    /// if provided before falling back to the remaining beacon nodes.
