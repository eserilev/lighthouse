...
impl<E: EthSpec> ExecPayload<E> for FullPayloadRef<'_, E> {
    ...
    fn blob_gas_used(&self) -> Result<u64, BeaconStateError> {
        match self {
            ...
            FullPayloadRef::Eip7805(inner) => Ok(inner.execution_payload.blob_gas_used),
...
impl<E: EthSpec> AbstractExecPayload<E> for FullPayload<E> {
    ...
    type Eip7805 = FullPayloadEip7805<E>;
...
pub struct BlindedPayload<E: EthSpec> {
    ...
    #[superstruct(only(Eip7805), partial_getter(rename = "execution_payload_eip7805"))]
    pub execution_payload_header: ExecutionPayloadHeaderEip7805<E>,
    #[superstruct(only(Fulu), partial_getter(rename = "execution_payload_fulu"))]
...
impl<'b, E: EthSpec> ExecPayload<E> for BlindedPayloadRef<'b, E> {
    ...
    fn withdrawals_root(&self) -> Result<Hash256, BeaconStateError> {
        match self {
            ...
            BlindedPayloadRef::Eip7805(inner) => {
                Ok(inner.execution_payload_header.withdrawals_root)
            }
...
impl<E: EthSpec> AbstractExecPayload<E> for BlindedPayload<E> {
    ...
    type Eip7805 = BlindedPayloadEip7805<E>;
    ...
    fn from(execution_payload_header: ExecutionPayloadHeader<E>) -> Self {
        match execution_payload_header {
            ...
            ExecutionPayloadHeader::Eip7805(execution_payload_header) => {
                Self::Eip7805(BlindedPayloadEip7805 {
                    execution_payload_header,
                })
            }
...
impl<E: EthSpec> From<BlindedPayload<E>> for ExecutionPayloadHeader<E> {
    fn from(blinded: BlindedPayload<E>) -> Self {
        match blinded {
            ...
            BlindedPayload::Eip7805(blinded_payload) => {
                ExecutionPayloadHeader::Eip7805(blinded_payload.execution_payload_header)
            }
