...
impl AllowNextEpoch {
    fn upper_bound_of(self, current_epoch: Epoch) -> Result<Epoch, BeaconStateError> {
        match self {
...
impl<E: EthSpec> BeaconState<E> {
    ...
    pub fn fork_name_unchecked(&self) -> ForkName {
        match self {
            ...
            BeaconState::Eip7805 { .. } => ForkName::Eip7805,
    ...
    pub fn canonical_root(&mut self) -> Result<Hash256, BeaconStateError> {
    ...
    fn shuffling_random_byte(i: usize, seed: &[u8]) -> Result<u8, BeaconStateError> {
        let mut preimage = seed.to_vec();
        ...
        hash(&preimage)
            .get(index)
            .copied()
            .ok_or(BeaconStateError::ShuffleIndexOutOfBounds(index))
    }

    /// Get two random bytes from the given `seed`.
    ///
    /// This is used in place of `shuffling_random_byte` from Electra onwards.
    fn shuffling_random_u16_electra(i: usize, seed: &[u8]) -> Result<u16, BeaconStateError> {
        let mut preimage = seed.to_vec();
        preimage.append(&mut int_to_bytes8(i.safe_div(16)? as u64));
        let offset = i.safe_rem(16)?.safe_mul(2)?;
        hash(&preimage)
            .get(offset..offset.safe_add(2)?)
            .ok_or(BeaconStateError::ShuffleIndexOutOfBounds(offset))?
            .try_into()
            .map(u16::from_le_bytes)
            .map_err(|_| BeaconStateError::ShuffleIndexOutOfBounds(offset))
    }
    ...
    pub fn latest_execution_payload_header(
        ...
        match self {
            ...
            BeaconState::Eip7805(state) => Ok(ExecutionPayloadHeaderRef::Eip7805(
                &state.latest_execution_payload_header,
            )),
    ...
    pub fn latest_execution_payload_header_mut(
        ...
        match self {
            ...
            BeaconState::Eip7805(state) => Ok(ExecutionPayloadHeaderRefMut::Eip7805(
                &mut state.latest_execution_payload_header,
            )),
    ...
    pub fn is_aggregator(
        ...
        let signature_hash_int = u64::from_le_bytes(
            signature_hash
                ...
                .ok_or(BeaconStateError::IsAggregatorOutOfBounds)?,
        );
    ...
    fn get_next_sync_committee_indices(
        &self,
        spec: &ChainSpec,
    ) -> Result<Vec<usize>, BeaconStateError> {
    ...
    pub fn get_sync_committee_duties(
        ...
        spec: &ChainSpec,
    ) -> Result<Vec<Result<Option<SyncDuty>, BeaconStateError>>, BeaconStateError> {
    ...
    pub fn get_block_root(&self, slot: Slot) -> Result<&Hash256, BeaconStateError> {
        ...
        self.block_roots()
            .get(i)
            .ok_or(BeaconStateError::BlockRootsOutOfBounds(i))
    }
    ...
    pub fn set_block_root(
        ...
        *self
            .block_roots_mut()
            .get_mut(i)
            .ok_or(BeaconStateError::BlockRootsOutOfBounds(i))? = block_root;
        Ok(())
    }

    /// Fill `randao_mixes` with
    pub fn fill_randao_mixes_with(&mut self, index_root: Hash256) -> Result<(), BeaconStateError> {
    ...
    pub fn set_randao_mix(&mut self, epoch: Epoch, mix: Hash256) -> Result<(), BeaconStateError> {
        ...
        *self
            .randao_mixes_mut()
            .get_mut(i)
            .ok_or(BeaconStateError::RandaoMixesOutOfBounds(i))? = mix;
        Ok(())
    }
    ...
    pub fn get_state_root(&self, slot: Slot) -> Result<&Hash256, BeaconStateError> {
        ...
        self.state_roots()
            .get(i)
            .ok_or(BeaconStateError::StateRootsOutOfBounds(i))
    }
    ...
    pub fn get_oldest_state_root(&self) -> Result<&Hash256, BeaconStateError> {
    ...
    pub fn get_oldest_block_root(&self) -> Result<&Hash256, BeaconStateError> {
    ...
    pub fn set_state_root(
        ...
        *self
            .state_roots_mut()
            .get_mut(i)
            .ok_or(BeaconStateError::StateRootsOutOfBounds(i))? = state_root;
        Ok(())
    }
    ...
    pub fn get_slashings(&self, epoch: Epoch) -> Result<u64, BeaconStateError> {
    ...
    pub fn set_slashings(&mut self, epoch: Epoch, value: u64) -> Result<(), BeaconStateError> {
        let i = self.get_slashings_index(epoch, AllowNextEpoch::True)?;
        *self
            .slashings_mut()
            .get_mut(i)
            .ok_or(BeaconStateError::SlashingsOutOfBounds(i))? = value;
        Ok(())
    }
    ...
    pub fn get_balance(&self, validator_index: usize) -> Result<u64, BeaconStateError> {
        self.balances()
            .get(validator_index)
    ...
    pub fn get_seed(
        ...
        let mix = {
            ...
            self.randao_mixes()
                .get(i_mod)
                .ok_or(BeaconStateError::RandaoMixesOutOfBounds(i_mod))?
        };
    ...
    ///
    /// Will error if the state lacks an `inactivity_scores` field.
    pub fn get_inactivity_score_mut(
        &mut self,
        validator_index: usize,
    ) -> Result<&mut u64, BeaconStateError> {
...
>>>>>>> 2ce6b51269708a1c28c69a3241028522ebc153df:consensus/types/src/state/beacon_state.rs
    ...
    pub fn get_epoch_participation_mut(
        ...
        if epoch == current_epoch {
            match self {
                ...
                BeaconState::Eip7805(state) => Ok(&mut state.current_epoch_participation),
            ...
            match self {
                ...
                BeaconState::Eip7805(state) => Ok(&mut state.previous_epoch_participation),
    ...
    pub fn advance_caches(&mut self) -> Result<(), BeaconStateError> {
    ...
    fn committee_cache_at_index_mut(
        &mut self,
        index: usize,
    ) -> Result<&mut Arc<CommitteeCache>, BeaconStateError> {
    ...
    #[instrument(skip_all, level = "debug")]
    pub fn update_tree_hash_cache<'a>(&'a mut self) -> Result<Hash256, BeaconStateError> {
    ...
    ///
    /// Initialize the tree hash cache if it isn't already initialized.
    pub fn update_validators_tree_hash_cache(&mut self) -> Result<Hash256, BeaconStateError> {
...
>>>>>>> 2ce6b51269708a1c28c69a3241028522ebc153df:consensus/types/src/state/beacon_state.rs
    ...
    pub fn rebase_on(&mut self, base: &Self, spec: &ChainSpec) -> Result<(), BeaconStateError> {
        ...
        match (&mut *self, base) {
            ...
            (Self::Electra(_), _) => (),
            (Self::Eip7805(self_inner), Self::Eip7805(base_inner)) => {
                bimap_beacon_state_eip7805_tree_list_fields!(
                    self_inner,
                    base_inner,
                    |_, self_field, base_field| { self_field.rebase_on(base_field) }
                );
...
impl<E: EthSpec> BeaconState<E> {
    ...
    pub fn num_fields_pow2(&self) -> usize {
        ...
        match fork_name {
            ...
            ForkName::Eip7805 => BeaconStateEip7805::<E>::NUM_FIELDS.next_power_of_two(),
    ...
    pub fn apply_pending_mutations(&mut self) -> Result<(), BeaconStateError> {
        match self {
            ...
            Self::Eip7805(inner) => {
                map_beacon_state_eip7805_tree_list_fields!(inner, |_, x| { x.apply_updates() })
            }
            Self::Fulu(inner) => {
    ...
    pub fn get_beacon_state_leaves(&self) -> Vec<Hash256> {
        ...
        match self {
            ...
            BeaconState::Eip7805(state) => {
                map_beacon_state_eip7805_fields!(state, |_, field| {
                    leaves.push(field.tree_hash_root());
                });
            }
...
impl From<RelativeEpochError> for BeaconStateError {
    fn from(e: RelativeEpochError) -> BeaconStateError {
        BeaconStateError::RelativeEpochError(e)
    }
}

impl From<ssz_types::Error> for BeaconStateError {
    fn from(e: ssz_types::Error) -> BeaconStateError {
        BeaconStateError::SszTypesError(e)
    }
}

impl From<bls::Error> for BeaconStateError {
    fn from(e: bls::Error) -> BeaconStateError {
        BeaconStateError::BlsError(e)
    }
}

impl From<tree_hash::Error> for BeaconStateError {
    fn from(e: tree_hash::Error) -> BeaconStateError {
        BeaconStateError::TreeHashError(e)
    }
}

impl From<merkle_proof::MerkleTreeError> for BeaconStateError {
    fn from(e: merkle_proof::MerkleTreeError) -> BeaconStateError {
        BeaconStateError::MerkleTreeError(e)
    }
}
...
impl From<milhouse::Error> for BeaconStateError {
    fn from(e: milhouse::Error) -> BeaconStateError {
        BeaconStateError::MilhouseError(e)
    }
}
...
impl<E: EthSpec> CompareFields for BeaconState<E> {
    fn compare_fields(&self, other: &Self) -> Vec<compare_fields::Comparison> {
        match (self, other) {
            ...
            (BeaconState::Eip7805(x), BeaconState::Eip7805(y)) => x.compare_fields(y),
