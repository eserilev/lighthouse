...
use crate::data_availability_checker_v2::error::Error;
pub use crate::data_availability_checker_v2::error::{
...
impl<T: BeaconChainTypes> DataAvailabilityChecker<T> {
    ...
    pub fn verify_kzg_for_available_payload(
        ...
        let block_data_required =
            self.data_columns_required_for_payload(&available_payload.payload);
    ...
    fn data_columns_required_for_payload(
        &self,
        payload: &SignedExecutionPayloadEnvelope<T::EthSpec>,
    ) -> bool {
        payload.message.blob_kzg_commitments.len() > 0
            && self.data_columns_required_for_epoch(payload.message.epoch())
    }
...
impl<E: EthSpec> AvailablePayload<E> {
        ...
        T: BeaconChainTypes<EthSpec = E>,
    {
        ...
        let columns_required = da_checker.data_columns_required_for_payload(&payload);
        ...
        match &column_data {
            ...
            AvailablePayloadData::DataColumns(data_columns) => {
                if !columns_required {
                    return Err(AvailabilityCheckError::InvalidAvailablePayloadData);
                }

                let mut column_indices = da_checker
                    .custody_context
                    .custody_columns_for_epoch(Some(payload.message.epoch()), &spec)
                    .iter()
                    .collect::<HashSet<_>>();
