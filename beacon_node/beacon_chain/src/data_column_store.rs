...
//! `DataAvailabilityChecker` v2 (for payload envelopes in Gloas+).
//!
//! ## Design
//!
//! - **Read operations**: Unified via the `DataColumnStore` trait
...
//!
//! After Gloas is fully activated and v1 is deprecated, this can be simplified.
...
use crate::payload_verification_types::AvailableExecutedPayload;
use std::sync::Arc;
use types::{ChainSpec, ColumnIndex, DataColumnSidecar, DataColumnSidecarList, EthSpec, Hash256, Slot};

// ============================================================================
// Unified Availability Outcome
// ============================================================================
...
pub enum AvailabilityOutcome<E: EthSpec> {
    ...
    /// Payload became available (Gloas+, from v2 checker)
...
pub enum ReconstructionOutcome<E: EthSpec> {
    ...
    /// Payload reconstruction result (Gloas+)
...
pub trait DataColumnStore<E: EthSpec>: Send + Sync {
    ...
    fn custody_context(&self) -> &Arc<CustodyContext<E>>;

    /// Returns all cached data columns for the given block root, if any.
    fn get_data_columns(&self, block_root: Hash256) -> Option<DataColumnSidecarList<E>>;
    ...
    fn is_data_column_cached(&self, block_root: &Hash256, data_column: &DataColumnSidecar<E>) -> bool;
    ...
    fn put_rpc_custody_columns(
        ...
        block_root: Hash256,
        slot: Slot,
        custody_columns: DataColumnSidecarList<E>,
    ...
    fn put_gossip_verified_data_columns<O: ObservationStrategy>(
        ...
        block_root: Hash256,
        slot: Slot,
        data_columns: Vec<GossipVerifiedDataColumn<E, O>>,
    ) -> Result<Self::Availability, AvailabilityCheckError>;
    ...
    fn put_kzg_verified_custody_data_columns(
        &self,
        block_root: Hash256,
        custody_columns: Vec<KzgVerifiedCustodyDataColumn<E>>,
    ) -> Result<Self::Availability, AvailabilityCheckError>;
...
// ============================================================================
// DataColumnRouter (Facade)
// ============================================================================

/// Router that directs data column operations to the appropriate DA checker based on fork.
///
/// This facade wraps both the legacy (v1) and Gloas (v2) DA checkers, providing:
/// - Unified read operations that query both checkers
/// - Fork-aware routing for write operations that return `AvailabilityOutcome`
///
/// After Gloas is fully activated and v1 is deprecated, this router can be simplified
/// to only use v2.
pub struct DataColumnRouter<E: EthSpec, V1, V2>
where
    V1: DataColumnStore<
        E,
        Availability = BlockAvailability<E>,
        ReconstructionResult = BlockReconstructionResult<E>,
    >,
    V2: DataColumnStore<
        E,
        Availability = PayloadAvailability<E>,
        ReconstructionResult = PayloadReconstructionResult<E>,
    >,
{
...
impl<E: EthSpec, V1, V2> DataColumnRouter<E, V1, V2>
where
    V1: DataColumnStore<
        E,
        Availability = BlockAvailability<E>,
    ...
    >,
{
    ...
    pub fn get_data_columns(&self, block_root: Hash256, slot: Slot) -> Option<DataColumnSidecarList<E>> {
    ...
    pub fn get_data_columns_any(&self, block_root: Hash256) -> Option<DataColumnSidecarList<E>> {
        self.v1
            .get_data_columns(block_root)
            .or_else(|| self.v2.get_data_columns(block_root))
    }

    /// Check if a data column is cached in either checker.
    pub fn is_data_column_cached_any(
        &self,
        block_root: &Hash256,
    ...
    // -------------------------------------------------------------------------
    // Write Operations (return unified AvailabilityOutcome)
    // -------------------------------------------------------------------------

    /// Insert RPC custody columns, routing to the correct checker based on fork.
    ///
    /// Returns a unified `AvailabilityOutcome` that can be processed by
    /// `BeaconChain::process_availability_outcome()`.
    pub fn put_rpc_custody_columns(
        &self,
        block_root: Hash256,
        slot: Slot,
        custody_columns: DataColumnSidecarList<E>,
    ) -> Result<AvailabilityOutcome<E>, AvailabilityCheckError> {
        if self.is_gloas(slot) {
            self.v2
    ...
    pub fn reconstruct_data_columns(
        ...
        block_root: &Hash256,
        slot: Slot,
    ) -> Result<ReconstructionOutcome<E>, AvailabilityCheckError> {
        if self.is_gloas(slot) {
            self.v2
    ...
    // -------------------------------------------------------------------------
    // Direct Access (for checker-specific operations)
    // -------------------------------------------------------------------------
...
// Note: The actual trait implementations would go in the respective DA checker modules.
// Here's what they would look like:
//
// // In data_availability_checker.rs (v1):
// impl<T: BeaconChainTypes> DataColumnStore<T::EthSpec> for DataAvailabilityChecker<T> {
//     type Availability = Availability<T::EthSpec>;  // BlockAvailability
...
//     fn cached_data_column_indexes(&self, block_root: &Hash256) -> Option<Vec<ColumnIndex>> { ... }
//     fn is_data_column_cached(&self, block_root: &Hash256, data_column: &DataColumnSidecar<T::EthSpec>) -> bool { ... }
//     fn put_rpc_custody_columns(&self, ...) -> Result<Self::Availability, AvailabilityCheckError> { ... }
//     fn put_gossip_verified_data_columns<O>(&self, ...) -> Result<Self::Availability, AvailabilityCheckError> { ... }
//     fn put_kzg_verified_custody_data_columns(&self, ...) -> Result<Self::Availability, AvailabilityCheckError> { ... }
//     fn reconstruct_data_columns(&self, ...) -> Result<Self::ReconstructionResult, AvailabilityCheckError> { ... }
// }
//
// // In data_availability_checker_v2.rs (same structure, different associated types):
// impl<T: BeaconChainTypes> DataColumnStore<T::EthSpec> for DataAvailabilityChecker<T> {
//     type Availability = Availability<T::EthSpec>;  // PayloadAvailability
//     type ReconstructionResult = DataColumnReconstructionResult<T::EthSpec>;
