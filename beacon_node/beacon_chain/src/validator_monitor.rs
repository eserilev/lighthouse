...
use types::{
    AttestationData, AttesterSlashingRef, BeaconBlockRef, BeaconState, BeaconStateError, ChainSpec,
    Epoch, EthSpec, Hash256, IndexedAttestation, IndexedAttestationRef, ProposerSlashing,
...
pub struct ValidatorMonitor<E: EthSpec> {
    ...
    // Attestation data produced by the attestation simulator at each slot.
    unaggregated_attestations: HashMap<Slot, AttestationData>,
...
impl<E: EthSpec> ValidatorMonitor<E> {
    ...
    pub fn set_unaggregated_attestation(&mut self, attestation_data: AttestationData) {
        ...
        self.unaggregated_attestations
            .insert(slot, attestation_data);
    }

    pub fn get_unaggregated_attestation(&self, slot: Slot) -> Option<&AttestationData> {
    ...
    fn process_unaggregated_attestations(&mut self, state: &BeaconState<E>, spec: &ChainSpec) {
        ...
        for slot in attested_slots {
            if let Some(data) = unaggregated_attestations.remove(&slot) {
