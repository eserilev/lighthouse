...
use execution_layer::{
    EngineCapabilities,
    http::{
        ENGINE_FORKCHOICE_UPDATED_V2, ENGINE_FORKCHOICE_UPDATED_V3, ENGINE_GET_INCLUSION_LIST_V1, ENGINE_GET_PAYLOAD_V2, ENGINE_GET_PAYLOAD_V3, ENGINE_GET_PAYLOAD_V4, ENGINE_GET_PAYLOAD_V5, ENGINE_NEW_PAYLOAD_V2, ENGINE_NEW_PAYLOAD_V3, ENGINE_NEW_PAYLOAD_V4
    },
};
    ...
    match fork {
        ...
        ForkName::Eip7805 => {
            if !capabilities.get_inclusion_list_v1 {
                missing_methods.push(ENGINE_GET_INCLUSION_LIST_V1)
            }

            // TODO(eip7805) check if were missing other capabilities
        }
        ForkName::Gloas => {
            if !capabilities.get_payload_v5 {
                missing_methods.push(ENGINE_GET_PAYLOAD_V5);
